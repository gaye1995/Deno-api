        ...
        if(authorization){
            ...
            if(!token){
                ...
                if(user){
                    ...
                    await User.insert();
                        User.updateSubscription(data.subscription);
                        User.setRole('Enfant');
                    c.response.status = 200;
                    return c.json({ error: false, message: "Votre enfant a bien été créé avec succés", User});
                }
            }catch (err){
                    c.response.status = 401;
                    return c.json({ error: true, message: err.message });
            ...
            return c.json({Error: true, message: "Token érroné"});
    }
    }
}
